...
func TestNewID(t *testing.T) {
	id1 := agentserver.NewID()
	id2 := agentserver.NewID()
...
func TestWriteEvent(t *testing.T) {
	...
	s.WriteHeader(agentserver.SessionMeta{SessionID: "s1"})
...
func TestLoadNonexistentFile(t *testing.T) {
	if _, err := agentserver.Load("/nonexistent/path.jsonl"); err == nil {
...
func TestRoundTripMultipleEvents(t *testing.T) {
	...
	s.WriteHeader(agentserver.SessionMeta{SessionID: "session-x"})
...
func TestAppendOnlyMode(t *testing.T) {
	...
	s := agentserver.NewStore(path)
	s.WriteHeader(agentserver.SessionMeta{SessionID: "first"})
...
func TestDefaultPath(t *testing.T) {
	p := agentserver.DefaultPath("/tmp/sessions", "session-id")
...
func TestDataIsValidJSON(t *testing.T) {
	...
	s.WriteHeader(agentserver.SessionMeta{SessionID: "s"})
	s.WriteEvent("data", data)

	loaded, _ := agentserver.Load(path)
...
func TestWriteAndLoadCompaction(t *testing.T) {
	...
	s.WriteEvent("thought_start", nil)
	if err := s.WriteCompaction(agentserver.CompactionRecord{Summary: "## Goal\nlearn repo", At: "2026-09-20T12:00:00Z"}); err != nil {
		t.Fatal(err)
	}

...
func TestDialFailsForMissing(t *testing.T) {
	if _, err := ipc.Dial("/nonexistent/path.sock"); err == nil {
		t.Error("expected error dialing non-existent socket")
	}
}

func TestIsRunningFalse(t *testing.T) {
	if ipc.IsRunning("/nonexistent/path.sock") {
...
func TestWriteServerPIDCreatesParentDir(t *testing.T) {
	...
	if err := ipc.WriteServerPID(path, 1234); err != nil {
...
func TestWriteServerPIDOverwrites(t *testing.T) {
	...
	if err := ipc.WriteServerPID(path, 111); err != nil {
		t.Fatal(err)
	}
	if err := ipc.WriteServerPID(path, 222); err != nil {
		t.Fatal(err)
	}
	got, err := ipc.ReadServerPID(path)
	if err != nil {
...
func TestRemoveServerPIDMissingIsNoop(t *testing.T) {
	if err := ipc.RemoveServerPID("/nonexistent/awp.tui.pid"); err != nil {

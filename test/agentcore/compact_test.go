...
func TestExtractFileOpsReadsWritesEdits(t *testing.T) {
	...
	ops := agentcore.ExtractFileOps(msgs)
	if !ops.Read["/a.go"] {
...
func TestFormatFileOperationsWrapsInXMLTags(t *testing.T) {
	got := agentcore.FormatFileOperations([]string{"/r.go"}, []string{"/m.go"})
...
func TestFormatFileOperationsEmptyReturnsEmpty(t *testing.T) {
	if got := agentcore.FormatFileOperations(nil, nil); got != "" {
		t.Errorf("got %q, want empty", got)
	}
}

func TestTruncateForSummaryKeepsShort(t *testing.T) {
	got := agentcore.TruncateForSummary("hello", 100)
	if got != "hello" {
...
func TestTruncateForSummaryCapsLong(t *testing.T) {
	...
	got := agentcore.TruncateForSummary(long, 200)
	if len(got) > 400 {
...
func TestSerializeForSummaryTruncatesToolResult(t *testing.T) {
	...
	out := agentcore.SerializeForSummary(msgs)
...
func TestEstimateTokens(t *testing.T) {
	if got := agentcore.EstimateTokens("hello world"); got != 3 {
...
func TestCountUserTurns(t *testing.T) {
	...
	if got := agentcore.CountUserTurns(msgs); got != 3 {
		t.Errorf("got %d, want 3", got)
	}
}
...
func TestFindCutPointKeepsRecent(t *testing.T) {
	...
	if got := agentcore.FindCutPoint(msgs, 2); got != 5 {
...
func TestFindCutPointKeepZero(t *testing.T) {
	...
	if got := agentcore.FindCutPoint(msgs, 0); got != 0 {

...
func runAgent(ctx context.Context, core llm.Core) {
	ag := agent.New(core, "MiniMax-M3").
		WithSystemPrompt("You are a helpful assistant. Use the available tools to answer user questions. Be concise.")
	...
	fmt.Print("> ")
	result, err := ag.Run(ctx, "What's the weather in Beijing and what time is it there?")
	if err != nil {

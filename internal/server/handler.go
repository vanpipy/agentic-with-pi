...
func (s *Server) dispatch(conn io.Writer, req *protocol.Request) {
	switch req.Method {
...
func (s *Server) handlePrompt(conn io.Writer, req *protocol.Request) {
	...
	events := s.agent.RunStream(s.ctx, params.Prompt)
	defer func() {
...
func slogDebug(msg string, err error) {

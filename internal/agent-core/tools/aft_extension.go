...
func aftCallTool(backend *AftBackend, name, description string, schema map[string]any) agentcore.Tool {
...
func aftCallToolNested(backend *AftBackend, name, description string, schema map[string]any) agentcore.Tool {
	return agentcore.ToolFunc{
		N: name,
		D: description,
		P: requireIntentSchema(name, schema),
